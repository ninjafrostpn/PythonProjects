...
class Birb:
    ...
    def show(self):
        self.temp += (w/4 - np.linalg.norm(self.pos - (w/2, h))) / 100
        self.temp = max(50, min(self.temp, 200))
        self.col = (self.temp, 0, 255 - self.temp)
...
class Flock:
    def __init__(self, population=10, neighbourno=2, tightness=20, strength=0.1, dynamicity=0.95):
        ...
        self.birbs = [Birb((np.random.random() * w, np.random.random() * h), 100)
    ...
    def show(self):
        ...
        for i, birb in enumerate(self.birbs):
            ...
            corrections = (birbvecs[i][closest].T
                           * ((birbdists[i][closest] - self.tightness * (birb.temp / 100)) / birbdists[i][closest])).T
...
F = Flock(200, 8, 70, strength=0.1, dynamicity=0.7)
